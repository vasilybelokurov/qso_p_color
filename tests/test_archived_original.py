"""The retired Legacy-only model, kept in models/archive/ as a regression.

It is no longer the model (see test_shipped_models.py), but the one model's
provenance depends on it -- its holdout blocks and its prior normalisation --
and the method note quotes its validated numbers as the benchmark. These tests
keep those numbers reproducible from the archived files.
"""
...
def score_readme_candidate(g_factor: float = 1.0, outlier: bool = False):
    ...
    dens = BackgroundSurfaceDensity.load(ROOT / "models/archive/original_legacy_south/background_density_south_global.json")
    prior = GridQSOPrior.load(ROOT / "models/archive/original_legacy_south/sigma_q_south.json")
    ...
    rows = score_candidates(
        ...
        outlier_model=(OutlierModel.load(ROOT / "models/archive/original_legacy_south/outlier_south.json")
                       if outlier else None),
    )
    return rows[0]


def test_archived_files_exist_and_load():
    for name in ("qso_south_full", "background_south_global",
                 "background_density_south_global", "sigma_q_south", "outlier_south"):
        p = ROOT / f"models/archive/original_legacy_south/{name}.json"
        assert p.exists(), f"{p} must stay in the archive"
    ...
    # the method note's predecessor appendix quotes these
    ...
    u = score_readme_candidate(outlier=True)
...
def test_archived_background_records_its_provenance():
    from qso_pcolor.background import BackgroundColourModel

    m = BackgroundColourModel.load(ROOT / "models/archive/original_legacy_south/background_south_global.json").meta
