...
\paragraph{A provenance correction.} The quasars in this figure were selected
by a script that re-derived the model's spatial holdout rather than reading it,
and re-derived it from a different sample (DESI alone, without de-duplication
or the quality cut), which yields $78$ candidate blocks where training had
$81$. Since \texttt{choice} is a function of the array it is given, the same
seed over a different block list is a different draw: seven of its sixteen
blocks were training blocks. \textbf{Three of the five quasars plotted here
therefore lie in blocks that were used in fitting.} We measured what that is
worth rather than assuming: comparing $\log p(\mathbf{c}\,|\,Q,z_{\rm spec})$
for in-sample against genuinely held-out quasars, matched in redshift and
reference magnitude over $45$ cells, gives an in-minus-out difference of
$+0.018$~nats. Against the $\log B \sim 5$--$15$ of these panels that is
negligible --- with $\sim\!10^2$ objects per free parameter per slice, an
object's own contribution to the density at its own location is not resolvable
--- so the numbers below are what they would have been either way. The split
is now recorded in the model file --- the seed and the sixteen block IDs --- and
read rather than reconstructed, so the claim and the figure cannot diverge
again. Every object then gets its \emph{own} local stellar model,
fitted in a $0.5^\circ$ cone centred on it with known quasars removed and the
masked area taken out of the density. The panels show three contours: the
quasar model at that object's target redshift, the stellar model fitted
globally across the southern footprint, and the stellar model local to that
object.
...
DESI DR1 quasars; three of the five fall in blocks used for fitting, for the
reason given in the text, which shifts their colour densities by $+0.018$~nats
against $\log B \sim 5$--$15$. \emph{Bottom:} five point sources,
...
\begin{tabular}{lrrrr}
\toprule
 & $z_0$ & $r$ & $\log \mathrm{BF}$ & $p_{\rm same}$ \\
\midrule
quasars & $1.42$ & $19.2$ & $+6.4$ & $5.4\times10^{-2}$ \\
        & $2.23$ & $20.3$ & $+7.5$ & $3.3\times10^{-2}$ \\
        & $1.84$ & $21.2$ & $+4.0$ & $9.5\times10^{-3}$ \\
        & $0.62$ & $21.1$ & $+4.8$ & $3.1\times10^{-3}$ \\
        & $2.85$ & $19.7$ & $\mathbf{-0.6}$ & $2.6\times10^{-5}$ \\
\midrule
field   & $1.84$ & $21.3$ & $-12.2$ & $1.2\times10^{-9}$ \\
        & $0.62$ & $19.5$ & $-4.2$  & $1.1\times10^{-6}$ \\
        & $2.23$ & $20.5$ & $-7.1$  & $1.5\times10^{-7}$ \\
        & $0.62$ & $19.3$ & $-6.1$  & $1.9\times10^{-7}$ \\
        & $0.62$ & $20.8$ & $-3.8$  & $1.8\times10^{-6}$ \\
\bottomrule
\end{tabular}
\end{center}

Four of the five quasars separate cleanly, and all five field sources do. Three
things in the figure are worth more than that headline.

\paragraph{The one failure is the expected one.} The quasar at $z=2.85$ returns
$\log\mathrm{BF}=-0.6$: its panel shows it sitting where the quasar locus
crosses the stellar sequence. At that redshift the optical colours of a quasar
and of a star coincide, and no amount of modelling recovers a distinction the
photometry does not contain. This is precisely the degeneracy $W1-W2$ exists to
break, and the clearest single argument for not running optical-only in
production.
...
$|b|>25^\circ$, where the field population is not varying rapidly, so the local
...
object here has $\log\mathrm{BF}=+7.5$ --- odds of about $1800$ to $1$ on colour
alone --- and still only $p_{\rm same}=0.03$, because the $\pm2000\kms$ window
