...
quasar above a wrong-redshift one with AUC $0.84$, separates quasars from
stars with AUC $0.98$, and under-predicts the same-redshift probability by a
...
wrong-redshift quasar $84\%$ of the time --- real information, and more than the
blur of $0.6$ in redshift led us to expect. Galaxies, not stars, are the
contaminant: some faint blue galaxies that DESI targeted look quasar-like in
these colours, but almost all of them are visibly extended in the imaging, so a
simple point-source check removes them. And the probability the scorer reports
...
of those are stars). One global background model --- the eight-field fit of
\S\ref{sec:examples} --- was used for every pair; the comparisons below are
quasar against quasar, where the background cancels, and the quasar/background
separation is large enough that its fine structure does not enter. Every number
...
$\mathbf{0.84}$; by $p(z\in W\,|\,\cvec,Q)$ alone it is $0.86$. The colours carry
...
that it does so with AUC $0.76$ against $0.84$.

\paragraph{The quasar finder.} Against spectroscopic stars the Bayes factor
has AUC $\mathbf{0.98}$, and $0.6\%$ of stars exceed the median same-redshift
quasar --- the ``factor $10^6$'' separation of \S\ref{sec:examples} in action.
Against spectroscopic galaxies it has AUC $0.81$ and $10.5\%$ exceed that
median. These are not random galaxies: they are objects DESI chose to observe
(emission-line, luminous-red and bright galaxies), and a third of them are
blue enough in $grz$ and faint enough in WISE to sit under the quasar locus.
They are also resolved: $3.1\%$ of the spectroscopic galaxies have Legacy
Surveys morphology \textsf{PSF}, against $91$--$94\%$ of the quasars and
$99\%$ of the stars. A morphology gate would remove $97\%$ of this contaminant
class at a cost of $6$--$9\%$ of quasars (the loss concentrated at low redshift,
where the host is resolved). The scorer is colour-only by design; this is the
cheapest improvement available to it.
...
\paragraph{Galaxies are the contaminant, not stars.} Colour alone separates
quasars from spectroscopic stars with AUC $0.98$ but from the galaxies DESI
targeted with only $0.81$; a tenth of those galaxies score above the median
same-redshift quasar. They are, however, resolved: $3\%$ are \textsf{PSF} in
the Legacy Surveys morphology, against $91$--$94\%$ of quasars. The scorer is
colour-only by design and does not use morphology; a \textsf{PSF} gate upstream
of it removes $97\%$ of this class at a cost of $6$--$9\%$ of quasars,
concentrated at low redshift where the host is resolved. That gate, and the
clustering factor above, are the two cheapest improvements available, and
