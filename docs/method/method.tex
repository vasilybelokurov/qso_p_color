...
Each object is treated as a separate problem. The quasars are drawn from the
model's \emph{reserved} spatial blocks, so none was used in fitting, and the
field sources come one from each of five independent random fields --- not five
from one patch, which would share a single stellar environment and be a random
sample of nothing. Every object then gets its \emph{own} local stellar model,
...
quasars from the model's reserved blocks. \emph{Bottom:} five point sources,
